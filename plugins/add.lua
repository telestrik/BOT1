-- ## begin add.lua by @ELTRASPEED1 ##

local function ELTRASPEED(msg)
...
local function ToOfan(msg, parts)
	if is_mod(msg) then
		local hash = "gp_lang:"..msg.to.id
		local lang = redis:get(hash)
		if parts[1]:lower() == 'unlock' or parts[1]:lower() == 'بازکردن قفل' then
			if parts[2]:lower() == 'add' or parts[2]:lower() == 'اد' then
		...
		if parts[1]:lower() == 'setadd' or 'تنظیم تعداد اد' and parts[2] then
		...
		if parts[1]:lower() == 'getadd'  or parts[1]:lower() == 'وضعیت قفل اد' then
